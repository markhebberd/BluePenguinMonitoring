...
needed to stand the service back up is in `/volume1/wildwatch`:
...
| Everything else | `kits/YYYY/MM/*.tar.gz` | secrets.php, nginx vhosts, php-fpm pool, cron, Postfix/Dovecot/Rspamd/Roundcube config, DKIM private keys, package list, firewall, live DNS records |
...
```bash
git clone /volume1/wildwatch/repo.git wildwatch     # or re-clone from GitHub if it still exists
```
...
# kit/files/<original absolute path> — copy each back to the same location
cp kit/files/var/www/wildwatch/shared/secrets.php /var/www/wildwatch/shared/
...
- **The nestcheck Play Store upload keystore and `nestcheck/secrets.props`.** These live on the
  ...
  cannot reach into the Mac. Copy them to `/volume1/wildwatch/mac-kit/` by hand:

  ```bash
  rsync -av ~/src/penguins/nestcheck/secrets.props \
            <upload-keystore.jks> <play-service-account.json> \
            admin@192.168.1.109:/volume1/wildwatch/mac-kit/
  ```
...
loaded into an empty database and the site is served from it, so `http://<nas>:8080/status/`
