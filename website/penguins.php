...
if ($chipId) {
    ...
    $stmt = $pdo->prepare("SELECT p.* FROM penguins p JOIN penguin_chips pc ON p.peng_num = pc.peng_num WHERE pc.chip_number = ?");
    $stmt->execute([$chipId]);
    $penguin = $stmt->fetch();
...
$sql = "SELECT
            p.peng_num, p.peng_num, p.tag_number, p.sex, p.life_stage,
            ...
            (SELECT COUNT(DISTINCT ps2.peng_num)
             FROM penguin_scans ps2
             JOIN penguin_scans ps3 ON ps2.observation_id = ps3.observation_id AND ps2.peng_num != ps3.peng_num
             WHERE ps3.peng_num = p.peng_num) as partner_count,
            (SELECT SUM(o2.chicks)
             FROM penguin_scans ps4
             JOIN observations o2 ON ps4.observation_id = o2.observation_id
             WHERE ps4.peng_num = p.peng_num AND o2.chicks > 0 AND o2.is_deleted = FALSE) as total_chicks_raised
        FROM penguins p
        LEFT JOIN penguin_scans ps ON ps.peng_num = p.peng_num
...
function getChips($pdo, $penguinId) {
    $stmt = $pdo->prepare("SELECT chip_number, chip_date, is_active FROM penguin_chips WHERE peng_num = ? ORDER BY chip_date");
